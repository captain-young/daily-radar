...
- **科技新闻**（2026-07-29 加入，用户指定）—— 每天 10 条科技前沿，**全科技范围、AI 优先排序**。
  一块玻璃里十行：中文标题（意译）+ 来源/HN 分 + 一句话摘要。
  源：HN 首页（Algolia API）+ TechCrunch RSS + The Verge RSS，全部免费无鉴权，均已实测可用。
  跨天去重靠 data JSON 里的 `news` 数组；摘要只写标题和原文撑得住的内容，标题不明就 WebFetch 原文。
  标签页从两个变三个，滑块三等分，GitHub 标签文案缩短为「GitHub」
