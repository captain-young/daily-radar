...
- **论坛热议**（2026-07-29 加入；当天先做的是「科技新闻」，用户看后说不会读编辑部新闻、
  要「国外论坛在吵什么」，遂改版）—— 每天 10 条，**AI 优先排序**。
  一块玻璃里十行：中文标题（意译）+ 「r/xx 日榜 #N · 评论 M」/「HN N 分 · 评论 M」+
  ...
  重试一次，再挂就放弃该源/该帖。
  「在讨论什么」必须来自实际抓到的评论区（帖 URL + `/.rss`；HN 用 Algolia items API），
  抓不到就只写帖子本身、不编观点。跨天去重靠 data JSON 的 `forum` 数组。
